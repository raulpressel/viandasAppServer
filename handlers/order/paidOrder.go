...
func PaidOrder(rw http.ResponseWriter, r *http.Request) {
	...
	modelOrder.Paid = true

	status, err := db.UpdateOrder(modelOrder)
	...
	if !status {
		http.Error(rw, "no se pudo acreditar el pago de la orden", http.StatusInternalServerError)
		return
	}
