...
func UploadTanda(rw http.ResponseWriter, r *http.Request) {
	...
	/* hs, err := time.ParseDuration(tandaDto.Tanda.HourStart)
	if err != nil {
		http.Error(rw, "Error en el formato de hora recibido "+err.Error(), http.StatusBadRequest)
		return
	}

	tandaModel.HourStart = hs.Minutes()

	he, err := time.ParseDuration(tandaDto.Tanda.HourEnd)
	if err != nil {
		http.Error(rw, "Error en el formato de hora recibido "+err.Error(), http.StatusBadRequest)
		return
	}

	tandaModel.HourEnd = he.Minutes() */
